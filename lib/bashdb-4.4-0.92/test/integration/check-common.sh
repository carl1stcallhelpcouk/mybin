...
export srcdir=/home/carl/bin/lib/bashdb-4.4-0.92/test/integration
[ -z "$SH" ] && export SH=/bin/bash

export top_builddir=/home/carl/bin/lib/bashdb-4.4-0.92
export top_srcdir=/home/carl/bin/lib/bashdb-4.4-0.92
export abs_top_srcdir=/home/carl/bin/lib/bashdb-4.4-0.92
...
check_output() {
  ...
  /usr/bin/diff  -w --unified -b "$testfile" "$rightfile" && rm -f "$testfile"
  return $?
}
...
run_debugger() {
  ...
  $SH -- ${top_builddir}/bashdb $dbg_opts -x "$cmdfile" "$debugged_script" $ARGS
  check_exit_status
}
